...
# --- what an amendment ends (this.i @ow6dzro4) --------------------------------
#
# The amender used to declare this set and be convicted where it differed from
# the computed one. That duty is gone: it gated nothing, so it existed only to
# create something that could be false. What survives is the computation, which
# is reported rather than judged — and the cases worth keeping are the ones about
# WHICH acts an amendment ends, which never depended on the declaration.


def amended_with_one_in_flight(founded):
    """A half-endorsed act, then an amendment replacing the clause under it."""
    tabled = founded.act("hire", "hire")
    founded.endorse(MARTA, tabled)
    amendment = founded.amend("amendment", BOARD_LAW)
    founded.endorse(MARTA, amendment)
    founded.endorse(DEV, amendment)
    return tabled, amendment


def test_an_amendment_ends_the_act_whose_clause_it_replaced(founded):
    """The hire was in flight and its clause moved, so the amendment ended it."""
    tabled, amendment = amended_with_one_in_flight(founded)

    ended = disturbed_by(founded.corpus, founded.corpus.event(amendment), founded.now)

    assert ended == (tabled,)


def test_an_amendment_is_affirmed_on_its_own_merits_whatever_it_ended(founded):
    """Ending live matters is a consequence of a lawful amendment, not a fault.

    This is the shape the removed duty inverted: the amendment below kills a
    pending act and is affirmed, because clause A2's slots were filled and
    nothing else is a reason to refuse it.
    """
    _, amendment = amended_with_one_in_flight(founded)
...
def test_an_amendment_nobody_carried_ends_nothing(founded):
    """Nothing is ended until an edition takes force (``this.i`` @xhtvuxnc)."""
    tabled = founded.act("hire", "hire")
    founded.endorse(MARTA, tabled)
    amendment = founded.amend("amendment", BOARD_LAW)
...
def test_a_settled_act_is_not_ended_by_a_later_amendment(founded):
    """Ending reaches acts in flight; one that has already settled stands forever."""

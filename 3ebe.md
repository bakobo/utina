...
- 2026-09-18T18:51Z Fixed in 994de71. Console now carries err_color alongside color, Console.over asks each stream separately via _takes_color, and render_error takes console.err_style. Pinned by four tests in tests/test_cli.py: tty stdout with redirected stderr, the reverse, an end-to-end check that a redirected stderr carries no escapes, and the hand-built-console fallback where err_color is None. this.i @ig3tc5om. Also added TERM=dumb as a no-color rung, below FORCE_COLOR.
